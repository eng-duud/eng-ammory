...
export default function ProjectForm({ projectId }: Props) {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (isEdit) {
        // sync gallery images
        const existingIds = (form.images).filter(i=>i.id).map(i=>i.id!);
        ...
        // new project — add gallery images
    ...
    <div className="max-w-4xl space-y-8">
      ...
      <div className="glass rounded-2xl p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <div>
            <label className={lbl} style={{color:"var(--text-faint)"}}>رابط المشروع الحي</label>
            <input className={inp} value={form.liveUrl} onChange={e=>setForm(f=>({...f,liveUrl:e.target.value}))} placeholder="https://..."/>
          </div>
          <div>
            <label className={lbl} style={{color:"var(--text-faint)"}}>رابط GitHub</label>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="flex items-center gap-3">
            <div className="relative">
              <input type="checkbox" id="featured" checked={form.featured} onChange={e=>setForm(f=>({...f,featured:e.target.checked}))} className="sr-only"/>
              <label htmlFor="featured" className="flex items-center gap-3 cursor-pointer">
                <div className="w-12 h-6 rounded-full relative transition-colors"
                  style={{background:form.featured?"var(--gold)":"var(--bg-400)"}}>
                  <div className="absolute top-1 w-4 h-4 rounded-full bg-white transition-all"
                    style={{left:form.featured?"calc(100% - 20px)":"4px"}}/>
                </div>
                <span className="font-dm text-sm" style={{color:"var(--text)"}}>مشروع مميز (يظهر في الرئيسية)</span>
              </label>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <div className="relative">
              <input type="checkbox" id="hidden" checked={form.hidden} onChange={e=>setForm(f=>({...f,hidden:e.target.checked}))} className="sr-only"/>
              <label htmlFor="hidden" className="flex items-center gap-3 cursor-pointer">
                <div className="w-12 h-6 rounded-full relative transition-colors"
                  style={{background:form.hidden?"#ef4444":"var(--bg-400)"}}>
                  <div className="absolute top-1 w-4 h-4 rounded-full bg-white transition-all"
                    style={{left:form.hidden?"calc(100% - 20px)":"4px"}}/>
                </div>
                <span className="font-dm text-sm" style={{color:"var(--text)"}}>إخفاء من الموقع</span>
              </label>
            </div>
          </div>
          <div>
            <label className={lbl} style={{color:"var(--text-faint)"}}>الترتيب</label>
            <input type="number" className={inp} value={form.order} onChange={e=>setForm(f=>({...f,order:+e.target.value}))}/>
          </div>
          <div>
            <label className={lbl} style={{color:"var(--text-faint)"}}>لون الخلفية</label>
            <div className="flex gap-2">
              <input type="color" value={form.color} onChange={e=>setForm(f=>({...f,color:e.target.value}))}
                className="w-12 h-10 rounded-lg border cursor-pointer" style={{borderColor:"var(--glass-border)"}}/>
              <input className="input-gold flex-1" value={form.color} onChange={e=>setForm(f=>({...f,color:e.target.value}))}/>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex gap-3 pb-8">
        <button onClick={save} disabled={saving}
          className="flex items-center gap-2 px-8 py-3 font-dm font-semibold rounded-lg transition-all disabled:opacity-60"
          style={{background:"var(--gold)",color:"var(--bg)"}}>
          {saving ? <Loader size={16} className="animate-spin"/> : null}
          {saving ? "جاري الحفظ..." : isEdit ? "حفظ التغييرات" : "إنشاء المشروع"}
        </button>
        <button onClick={()=>router.push("/studio-x7k/projects")}
          className="px-6 py-3 font-dm rounded-lg transition-all"
